...
if provider == "finnhub":
    st.info(
        "Finnhub currently supplies the latest quote only. Historical performance and market "
        "regime require Twelve Data."
    )
        ...
        cards.append(
            {
                "symbol": symbol,
                "price": close,
                "1D": (close / bars[-2]["close"] - 1) * 100 if len(bars) >= 2 else None,
                "1W": (close / bars[-6]["close"] - 1) * 100 if len(bars) >= 6 else None,
                "1M": (close / bars[-22]["close"] - 1) * 100 if len(bars) >= 22 else None,
